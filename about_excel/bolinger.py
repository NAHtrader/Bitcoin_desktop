...
# ㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡ엑셀 불러오기ㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡ
    df = pd.read_excel('C:/Users/whdtlr/Bitcoin_desktop/march_15m/'+coin[1]+'.xlsx')
    num_index = len(df.index)
# ㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡ반복문을 통해 추가하기ㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡ
# 카운팅 쉽게 하기 위해 앞에 400개 잘라내버리기
    df['moving average price']=df['Average Trade price'].rolling(window=20).mean()
    df['std']=df['Average Trade price'].rolling(window=20).std()
    df['low_bolinger']=(df['moving average price']-df['std']*2)

# ㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡ엑셀 저장ㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡ

    df.to_excel(excel_writer='C:/Users/whdtlr/Bitcoin_desktop/march_15m/bolinger/'+coin[1]+'.xlsx',index=0)
    print("Work Done")
