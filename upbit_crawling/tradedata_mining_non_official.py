...
for coin in coin_names:
    box = {}
    for day in days:
        r = requests.get("https://crix-api-endpoint.upbit.com/v1/crix/candles/minutes/15?code=CRIX.UPBIT."+coin[0]+"&count=96&to=2021-03-"+day+"%2009:00:00")
        tradedata = r.json()

        for trade in reversed(tradedata):
            time = trade['candleDateTime']
            timedata = time[0:10] +" "+ time[11:19]
    ...
    transposed_tradedata.to_excel(excel_writer='C:/Users/whdtlr/Bitcoin_desktop/march_15m/'+coin[1]+'.xlsx')
    print(coin[1])
#     # # cycle+=1
#     # # if cycle%5==0:
#     # #     sleep(60)
